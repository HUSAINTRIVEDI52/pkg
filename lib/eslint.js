...
exports.setupESLintConfig = async () => {
  ...
  // Detect installed ESLint version (needed for migration logic below)
  let installedEslintMajor = 9; // assume modern by default
  try {
    ...
    if (await fs.pathExists(eslintPkgPath)) {
      ...
      installedEslintMajor = parseInt(eslintPkg.version.split('.')[0], 10);
    }
  } catch { /* ignore */ }

  const isLegacyConfig = existingConfigFile && (
    existingConfigFile === '.eslintrc.json' ||
    existingConfigFile === '.eslintrc.js' ||
    existingConfigFile === '.eslintrc.cjs' ||
    existingConfigFile === '.eslintrc.yaml' ||
    existingConfigFile === '.eslintrc.yml' ||
    existingConfigFile === '.eslintrc'
  );

  if (hasConfig) {
    // If the project has a LEGACY config but ESLint v9+ is installed,
    // the legacy config will be silently ignored / error — migrate to flat config.
    if (isLegacyConfig && installedEslintMajor >= 9) {
      logInfo(`Legacy ESLint config (${existingConfigFile}) found but ESLint v${installedEslintMajor} is installed — migrating to flat config...`);

      const flatConfigPath = path.join(projectRoot, 'eslint.config.mjs');
      const templatePath = path.resolve(__dirname, '../templates/eslint.config.mjs');

      if (await fs.pathExists(templatePath)) {
        await fs.copy(templatePath, flatConfigPath);
        logSuccess(`Created eslint.config.mjs (flat config). Legacy ${existingConfigFile} kept as backup.`);
      } else {
        logInfo('Flat config template not found — skipping migration.');
      }
      return;
    }
    ...
    // Only merge into .eslintrc.json (legacy format, ESLint v8) — flat configs manage themselves
    if (existingConfigFile === '.eslintrc.json' && installedEslintMajor < 9) {
      ...
      logInfo(`Existing config is ${existingConfigFile || 'package.json'} — no merge needed.`);
    }
