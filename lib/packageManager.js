...
exports.installAllRequiredDependencies = async () => {
  ...
  if (isVite) {
    // Attempt to match the project's vitest version for coverage-v8
    let vitestVer = '*';
    try {
      const pkg = await fs.readJSON(pkgPath);
    ...
    // Use the same version/range for coverage-v8 to avoid ERESOLVE conflicts
 ...
 *
 * Strips version tags from package names (e.g. '@vitest/coverage-v8@^1.0.0' -> '@vitest/coverage-v8')
 * Correctly handles scoped packages.
 */
exports.getPackageBaseName = (pkg) => {
  if (pkg.startsWith('@')) {
...
exports.installDevDependency = async (pkg) => {
  ...
  // Added --legacy-peer-deps to bypass common peer dependency conflicts in user projects
