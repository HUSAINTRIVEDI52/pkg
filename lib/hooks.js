...
if [ -f "./node_modules/.bin/eslint" ]; then
  ...
  if [ -f "package.json" ]; then
    ...
    PKG_MANAGER="npm"
    DLX_CMD="npx"
    if [ -f "pnpm-lock.yaml" ]; then PKG_MANAGER="pnpm"; DLX_CMD="pnpm dlx";
    elif [ -f "yarn.lock" ]; then PKG_MANAGER="yarn"; DLX_CMD="yarn dlx";
    elif [ -f "bun.lockb" ]; then PKG_MANAGER="bun"; DLX_CMD="bunx"; fi
...
if [ -n "$ESLINT_BIN" ]; then
  ...
  run_cs_setup() {
    ...
    elif command -v cs-setup >/dev/null 2>&1; then
      cs-setup "$@"
    else
      echo "[cs-setup] Binary not found locally. Run '$DLX_CMD cs-setup init' to set up."
      return 1
    fi
  }
  ...
  if [ $HAS_CONFIG -eq 1 ]; then
    ...
    echo "   Run '$DLX_CMD cs-setup init' in your project to set up ESLint."
  fi
else
...
if [ ! -f "sonar-project.properties" ]; then
  ...
  if grep -q "^sonar.login=REPLACE_WITH_YOUR_TOKEN" sonar-project.properties || \\
    ...
    if [ -f "./node_modules/.bin/sonar-scanner" ]; then
      ...
      SONAR_BIN="$DLX_CMD sonar-scanner"
    fi
