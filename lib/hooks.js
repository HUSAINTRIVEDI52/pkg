...
'
      ...
      ESLINT_OUTPUT=$($ESLINT_BIN "$@" 2>&1)
      ESLINT_EXIT=$?

      echo "$ESLINT_OUTPUT"

      # If ESLint can't find a config, warn but don't block the commit
      if echo "$ESLINT_OUTPUT" | grep -q "ESLint couldn't find"; then
        echo ""
        echo "⚠️  [ESLint] No ESLint config found — skipping lint check."
        echo "   Run 'npx cs-setup init' in your project to create one."
