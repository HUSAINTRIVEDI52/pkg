...
if (!pkg.scripts['test:smoke']) {
  ...
  // 4. Ensure 'check-hooks' is run before tests and start (only in production/CI)
  ...
  // Skip if DEV_MODE is set to allow normal development
  const checkCmd = '[ "$DEV_MODE" != "true" ] && [ "$SKIP_HOOKS" != "true" ] && npx --no-install cs-setup check-hooks 2>/dev/null || true && ';
