...
if [ ! -f "sonar-project.properties" ]; then
  echo "[SonarQube] sonar-project.properties not found — skipping."
    ...
    # 1. Properly format the inclusions (using double backslashes for the template)
    SONAR_INCLUSIONS=$(echo "$STAGED_FILES" | tr '\\\\n' ',' | sed 's/,$//')
    
    # 2. If no specific files are found, default to scanning the current directory
    if [ -z "$SONAR_INCLUSIONS" ]; then SONAR_INCLUSIONS="."; fi

    echo "[SonarQube] Scanning: $SONAR_INCLUSIONS"

    # 3. Run with corrected flags and a space between them
    # We put the wait flag FIRST to ensure it is registered correctly
    ...
    echo "✅ [SonarQube] Analysis passed & Quality Gate OK. ✔"
  fi
fi
