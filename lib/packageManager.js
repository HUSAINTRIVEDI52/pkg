...
exports.installAllRequiredDependencies = async () => {
  ...
  // Fallback: If not found in root (e.g. monorepos), quickly scan file tree for ANY .ts/.tsx file.
  // This is better than `git ls-files` because it catches completely untracked/newly created projects.
  if (!hasTypeScript) {
    try {
      const dirsToSearch = [process.cwd()];
      let count = 0;
      while (dirsToSearch.length > 0 && count < 200) { // Limit depth to prevent infinite loops
        const currentDir = dirsToSearch.pop();
        count++;
