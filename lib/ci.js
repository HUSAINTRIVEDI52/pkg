...
exports.setupCIWorkflow = async () => {
  const sourceDir = path.resolve(__dirname, '../templates/.github');
  const targetDir = path.join(process.cwd(), '.github');

  if (!await fs.pathExists(sourceDir)) {
    logInfo("Templates .github folder not found.");
    return;
  }
  ...
  logInfo("Copying templates/.github to project's .github folder...");
