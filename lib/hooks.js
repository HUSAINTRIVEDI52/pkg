...
LINT_FILES=$(find . -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" -name "*.js" -o -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" -name "*.jsx" -o -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" -name "*.ts" -o -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" -name "*.tsx" -o -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" -name "*.mjs" -o -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" -name "*.cjs" 2>/dev/null)

if [ -n "$LINT_FILES" ]; then
  echo "[ESLint] Linting entire project..."
else
  echo "[ESLint] No JS/TS files found in project. Skipping."
fi
...
if [ -n "$LINT_FILES" ]; then
  ...
  if [ -n "$ESLINT_BIN" ]; then
    ...
    if [ $HAS_CONFIG -eq 0 ]; then
      ...
      # Re-check on disk after auto-heal attempt
      if [ -f "eslint.config.js" ] || [ -f "eslint.config.mjs" ] || [ -f "eslint.config.cjs" ] || \\
         [ -f ".eslintrc.js" ] || [ -f ".eslintrc.cjs" ] || [ -f ".eslintrc.yaml" ] || \\
         [ -f ".eslintrc.yml" ] || [ -f ".eslintrc.json" ] || [ -f ".eslintrc" ]; then
        HAS_CONFIG=1
    ...
    if [ $HAS_CONFIG -eq 1 ]; then
      ...
      # Collect all files into positional args (handles spaces in filenames)
      # Use IFS to split the variable by newlines into positional parameters
      # set -f (disable globbing) ensures filenames with * or ? are not expanded
      OLD_IFS="$IFS"
      IFS='
'
      set -f
