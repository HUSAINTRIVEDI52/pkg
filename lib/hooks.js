...
if [ ! -f "sonar-project.properties" ]; then
  echo "[SonarQube] sonar-project.properties not found — skipping."
    ...
    # --- ONLY THESE 4 LINES ARE NEW/FIXED ---
    CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    SONAR_INCLUSIONS=$(echo "$STAGED_FILES" | tr '\\\\n' ',' | sed 's/,$//')
    if [ -z "$SONAR_INCLUSIONS" ]; then SONAR_INCLUSIONS="."; fi

    echo "[SonarQube] Branch: $CURRENT_BRANCH"
    echo "[SonarQube] Scanning: $SONAR_INCLUSIONS"

    # FIXED COMMAND: Separate flags with spaces and added branch name
    $SONAR_BIN -Dsonar.qualitygate.wait=true -Dsonar.branch.name="$CURRENT_BRANCH" -Dsonar.inclusions="$SONAR_INCLUSIONS"
    # ----------------------------------------
