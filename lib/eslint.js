...
exports.setupESLintConfig = async () => {
  ...
  // Extend .eslintrc.json with TypeScript support if needed
  if (hasTypeScript && !useFlatConfig) {
    try {
      const config = await fs.readJSON(correctTargetPath);
      ...
      await fs.writeJSON(correctTargetPath, config, { spaces: 2 });
