...
exports.ensureProjectScripts = async () => {
  ...
  if (isPostInstall) {
    logInfo("Post-install detected: skipping project script standardization.");
    return;
  }
  ...
  if (!pkg.scripts['test:all']) {
    ...
    const runCmd = manager === 'yarn' ? 'yarn' : `${manager} run`;
    pkg.scripts['test:all'] = `${runCmd} test:smoke && ${runCmd} test:newman`;
    logInfo(`Creating "test:all" script using ${manager}.`);
    changed = true;
  }
...
exports.ensurePackageLock = async () => {
  const isPostInstall = process.env.npm_lifecycle_event === 'postinstall';
  if (isPostInstall) {
    logInfo("Post-install detected: skipping automatic lockfile generation.");
    return;
  }
  ...
  for (const item of lockFiles) {
    if (await fs.pathExists(path.join(process.cwd(), item.file))) {
      logSuccess(`Lock file found (${item.file}).`);
      return;
    }
  }

  const manager = detectPackageManager();
  logInfo(`No lock file found — running ${manager} install to generate one...`);
  try {
    execSync(`${manager} install`, { stdio: 'inherit', cwd: process.cwd() });
    logSuccess(`${manager} install completed successfully.`);
  } catch {
    logError(`Failed to run ${manager} install. Run it manually.`);
  }
};
