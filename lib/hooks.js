...
# ---------------------------------------------------------------
# SonarQube — Universal Robust Scanner (Community & Developer)
# ---------------------------------------------------------------
echo ""
...
if [ ! -f "sonar-project.properties" ]; then
  ...
  if grep -q "^sonar.login=REPLACE_WITH_YOUR_TOKEN" sonar-project.properties || \
    ...
    # 1. Robust Inclusion Formatting: No more empty inclusions
    # tr '\\\\n' is required to handle the JS-to-Shell string conversion
    SONAR_INCLUSIONS=$(echo "$STAGED_FILES" | tr '\\\\n' ',' | sed 's/,$//')
    
    # 2. Safety Fallback: If no specific files, scan current directory
    if [ -z "$SONAR_INCLUSIONS" ]; then SONAR_INCLUSIONS="."; fi

    echo "[SonarQube] Analyzing: $SONAR_INCLUSIONS"

    # 3. Execution: No branch flag = Highest Compatibility
    # This automatically updates the 'Main' branch on the server
    $SONAR_BIN -Dsonar.qualitygate.wait=true -Dsonar.inclusions="$SONAR_INCLUSIONS"
